...
- Build upstream SGLang native wheels without build isolation so the build uses the qualified nightly PyTorch, not the stable version pinned in the kernel package. Current nightly headers require C++20, and upstream CMake policies require a newer CMake than the system installation; pass the CUDA toolkit version explicitly so SM120 kernels remain enabled. Set `SGL_KERNEL_SM120_ONLY=ON` for the SM120 production wheel; that option omits architecture-specific libraries which have no SM120 implementation and must not be used for wheels targeting other GPU architectures.
...
- To empty all HiCache tiers without restarting an idle SGLang server, run these commands in order (the flush waits up to 120 seconds for requests to become idle):
  ...
  ```bash
  curl --fail-with-body -sS -X POST 'http://127.0.0.1:8000/flush_cache?timeout=120'
  curl --fail-with-body -sS -X POST 'http://127.0.0.1:8000/hicache/storage-backend/clear'
  ```
